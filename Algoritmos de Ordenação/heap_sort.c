...
void heapify(int v[], int tam, int i) {
    ...
    if (esq < tam) { comparacoes++; if (v[esq] > v[maior]) maior = esq; }
    if (dir < tam) { comparacoes++; if (v[dir] > v[maior]) maior = dir; }
    if (maior != i) { troca(v, i, maior); heapify(v, tam, maior); }
}

void heap_sort(int v[], int tam) {
    for (int i = tam/2-1; i >= 0; i--) heapify(v, tam, i);
    for (int i = tam-1; i > 0; i--)   { troca(v, 0, i); heapify(v, i, 0); }
}
